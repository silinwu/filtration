...
int main (int argc, char **argv) try
{
    ...
    int Pny = 3; 
    int pnx = 9;//small particle number
    int pny = 11; 
	double l  = 10;     //小颗粒之间表面间距
	...
	std::cout<<"1 含水率 = "<<(((l+2*R)*(l+2*R)-3.14*R*R)/(3.14*R*R*2.7))<<std::endl;
	std::cout<<"2 滤层孔径与颗粒平均直径比值 AOS/D = "<<pdx/(2*R)<<std::endl;
	std::cout<<"3 过滤介质排数 = "<<Pny<<std::endl;

	std::cout<<"4 泥浆颗粒半径 R = "<<R<<std::endl;
    std::cout<<"5 滤层颗粒半径 RR = "<<RR<<std::endl;
	std::cout<<"nx1="<<std::ceil(Pnx*(RR*2+pdx))<<"; nx2="<<std::ceil(pnx*(R*2+l))<<std::endl;
	std::cout<<"6 计算边界x的长度 nx="<<nx<<"; 计算边界y的长度 ny="<<ny<<"; 计算边界z的长度 nz="<<nz<<std::endl;
	std::cout<<"7 重力gy = "<<gy<<std::endl;
    ...
    my_dat.rho1 = 1.0135;
    my_dat.rho2 = 0.9865;
	std::cout<<"8 压差 = "<<100*(my_dat.rho1-my_dat.rho2)/3<<"kPa"<<std::endl;
    my_dat.pnx = pnx;
    my_dat.pny = pny;
    my_dat.sy = sy;
	my_dat.ny = ny;
...
//		dom.Particles[ip].l = 3.04e-10/ratiol;
    ...
    //solving
    dom.SolveIBM( Tf, dtout, "test23", Setup, NULL);
