...
prefix = "/media/zhuwei/My Book/filtration/D3/D3__"                      #********************************
numberhdf5 = 301                                                         #********************************
	...
	for n in range(cakelengthmaxnum):
		...
		for i in range(Np):
			if posy[i] >= cakelengthBi-0.2*R and posy[i] <= cakelengthTi+0.2*R and abs(VPy[i]) <= 0.01:    # we add a condition that (VPy[i]) <= 0.01, which is mean when meet this condition, the veclosity of the particle is very low, this is convient for testting the cake formed by floc, 0.01 is an experience point.        
				N +=1
		if N <= fnx * int((RRR/R)):                                                           #if the number of the y position of the particles between cakelenthB and cakelenthT is less than pnx +3, then define the length of the cake
			cakelength = n*2*R
			cakelengthr = cakelength*ratiol                                                   #the real cake length(m)
			Densitycaketop1 = Density[cakebottom - 1 + int(0.8*cakelength)-1,:]                  #the density cake at top value  [list], 0.8 is an experience value, for the data is more stable
			Densitycaketop1 = np.mean(Densitycaketop1) 
			Densitycaketop2 = Density[cakebottom - 1 + int(0.8*cakelength),:]                  #the density cake at top value  [list], 0.8 is an experience value, for the data is more stable
			Densitycaketop2 = np.mean(Densitycaketop2)
			Densitycaketop3 = Density[cakebottom - 1 + int(0.8*cakelength)+1,:]                  #the density cake at top value  [list], 0.8 is an experience value, for the data is more stable
