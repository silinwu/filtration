...
sy = (Pny-1)*(3**0.5*RR+pdy) + 2*RR + 0.5*l + R + 20
dtout = 5e3
cakebottom = int(round(sy - (0.5*l + R))) + 1                                                 #the boundary bewteen cake and filter
filterlength = (Pny-1)*(3**0.5*RR+pdy) + 2*RR                                                 #the length of the filter
Filtrationvolumnr = 0
###################read hdf5 ###########################################################################################################################################################################
prefix = "/home/foggy/dlvo-master/test_1_"
numberhdf5 = 43
...
###########caculate the length of the cake, the pressure drop of the cake and output to the excel built before##########################################################################################
	cakelengthmaxnum = int((ny-(sy-(0.5*l + R)))/(2*R))                                       #the max piece number of the cake during the simulation test
	...
	for n in range(cakelengthmaxnum):
		...
		N = 0
		for i in range(Np):
			if posy[i] >= cakelengthBi-0.2*R and posy[i] <= cakelengthTi+0.2*R:               
				N +=1
