...
void Setup(LBM::Domain &dom, void *UD)
{
    ...
    for(size_t ip=0; ip<dom.Particles.size(); ++ip)
    {
        ...
        if(dom.Particles[ip].X(1)>dat.ny-dat.lll+dat.R)
        {
            #pragma omp atomic
            ipp++;
        }
            ...
            for(int i=0; i<dat.fnx; ++i)
            {
                ...
                for(int k=-dat.nfloc; k<dat.nfloc+2; k=k+2)
                {
                    for (int kk=0; kk<nnfloc+1; ++kk)
                    {
                        ...
                        double fy =  dat.ny- ((std::sqrt(3)/3)*dat.nfloc+1)*dat.R - (std::sqrt(3)/3)*dat.nfloc*dat.R + std::sqrt(3)*dat.R*kk;
                        pos = fx, fy , 0;
            ...
            for(int i=0; i<dat.fnx; ++i)
            {
                ...
                for(int k=-dat.nfloc; k<dat.nfloc+2; k=k+2)
                {
		    		for(int kk=0; kk<dat.nfloc+1; ++kk)
                    {
		    		    ...
		    		    double fy = dat.ny - dat.nfloc*dat.R - dat.R + (-dat.nfloc)*dat.R+2*dat.R*kk;
		    		    pos = fx, fy , 0;
		    		    dom.Particles.push_back(DEM::Disk(-pnum, pos+dxr, v, w, dat.rhos, dat.R, dom.dtdem));
		    		    pnum++;
                        dom.Particles.back().Ff =  0.0, -M_PI*dat.R*dat.R*(dat.rhos/1.0-1)*dat.g,0.0, 0.0;
    ...
    if(dat.shape==6)
    {
        ...
        {
            double ffy = 0;
            double fy = dat.ny - ((std::sqrt(3))*dat.nfloc+1)*dat.R;
            for(int i=0; i<dat.fnx; ++i)
            {
                Vec3_t dxr(random(-0.45*dat.RRR,0.45*dat.RRR),random(0*dat.RRR,0*dat.RRR),0.0);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // dom.modexy = 1;
    //solving
    dom.SolveIBM( Tf, dtout, "testflocfltitrationtest_", Setup, NULL);
}MECHSYS_CATCH