...
#######particle################################################################################################
R = 10																	 # *****************
#pnx = 7                                                        		 #used when particle filtration  *****************
#pny = 3																 #used when particle filtration  *****************
#l  = 24.84																 #used when particle filtration  *****************
...
if shape == 3:
	...
	watercontentinitial = ((lll*lll-3.14*R*R*(3*(1+nfloc)*nfloc+1))/(3.14*R*R*(3*(1+nfloc)*nfloc+1)*2.7))         # shape =6
	RRR = (nfloc*2+1)*R 
#######environment#############################################################################################
dtout = 500                                                              #time step ***************
ratiol = 1e-6
...
MMMM = 0
###################read hdf5 ###########################################################################################################################################################################
#prefix = "/home/foggy/dlvo-master0.9/F1__"
prefix = "/media/zhuwei/My Book/filtration/D1/D1__"
numberhdf5 = 380
for data in range(numberhdf5 - 1):
	...
	if MMMM == 0 :                                                                              #different the filter particle and slurry particle, to increase the speed 
		for i in range(Np):
			if(isf[i]<0):
				MMMM += 1
			else:
...
###########caculate the length of the cake, the pressure drop of the cake and output to the excel built before##########################################################################################
	...
	for n in range(cakelengthmaxnum):
		...
		N = 0
		for i in range(MMMM-1, Np):
			if posy[i] >= cakelengthBi-0.2*R and posy[i] <= cakelengthTi+0.2*R and abs(VPy[i]) <= 0.01:    # we add a condition that (VPy[i]) <= 0.01, which is mean when meet this condition, the veclosity of the particle is very low, this is convient for testting the cake formed by floc, 0.01 is an experience point.        
				N +=1
		if N <= fnx * int((RRR/R)):                                                           #decide the top of the cake, if this condition is not satified, come to the top of the cake1, this value is set based on experience
			cakelength = n*2*R                                                                #2019-07-29, this cakelength isn't very good, so we creat the cakelengthrevise, the definition of the cakelengthrevise is the average of the top and the bottom of the cake
			Ncake = int(Nx/10)                                                         		  #Ncake is for optimizing the cakelengthrevise
			cakelengthT = cakebottom + cakelength                                             #cakelengthT is for optimizing the cakelengthrevise
	#		yMax = cakelengthT
			cakeaccumulate = 0
			for m in range(Ncake+1):                                                          #x=-20~0~20;-10~10~30; 0~20~40;10~30~50......m is used to divide the x area 
				if n == 0:                                                                    #if n = 0, it means that cake hasn't form, there is no need to start the loop, to speed this program.
					break
				NNNN = 0
				for j in range(int(cakelength/5)):
					yMaxx = cakelengthT - 5*j                                                   #from the top of the cake to find the bottom of the cake, the length of the cake is caculated by (top+bottom)/2
					if yMaxx <= cakebottom:
						cakeaccumulate = cakeaccumulate + cakebottom
						break
					for k in range(MMMM-1, Np):                                                       #"posy[k] <= cakelengthT and posy[k] >= cakebottom" mean that the particle should be in this y area; "posx[k] > (10*m-20) and posx[k] <= (10*m+20)" mean that in this x area; "((yMaxx - posy[k])**2 + (10*m - posx[k])**2) <= 100" mean that we have touch the bottom of the cake in this x area; if yMaxx <= yMax, mean that we get a much lower position in y direction, final, we can get the bottom of the cake
						if posy[k] <= cakelengthT and posy[k] >= cakebottom and posx[k] > (10*m-20) and posx[k] <= (10*m+20) and abs(VPy[k]) <= 0.01 and ((yMaxx - posy[k])**2 + (10*m - posx[k])**2) <= 169 :
							cakeaccumulate = cakeaccumulate + yMaxx - 3
							NNNN += 1
							break
					if NNNN > 0:
						break
			if n == 0:
				...
				cakelengthrevise = (cakeaccumulate/(Ncake+1))-cakebottom	                      #cakelengthrevise: the average length of the cake, is better than "cakelength"
				cakelengthreviser = cakelengthrevise*ratiol                                   #the real cake length(m)
			Densitycaketop = Density[cakebottom - 1 + cakelengthrevise,:]                     #the density cake at top value  [list]
			...
			sheet1.write(data+1,1,cakelengthreviser*1000000)                                  #output the reality cake length(um)
			sheet1.write(data+1,2,cakelengthreviser)                                          #output the reality cake length(m)
...
###########caculate the Porosity of the cake and output to the excel built before###################################################################################################################################	
			...
			cakelengthT = cakebottom + cakelengthrevise                                       #the top of the cake
			...
			for i in range(MMMM-1, Np):	
				if (cakelengthT - posy[i]) >= R and (posy[i] - cakelengthB) >= R:             #particles inside the cake
					...
					Dlength = (posy[i] - cakelengthB)
					particleinsidecakearea += 0.5*(math.pi)*R*R - (R*R*math.acos(Dlength/R)-Dlength*((R*R-Dlength*Dlength)**0.5))
					Dlength = (cakelengthT - posy[i])
					particleinsidecakearea += 0.5*(math.pi)*R*R - (R*R*math.acos(Dlength/R)-Dlength*((R*R-Dlength*Dlength)**0.5))
				else:
					particleinsidecakearea += 0	
			cakearea = cakelengthrevise * Nx
...
###########caculate the hydaulic conductivity of cake(m/s) and output to the excel built before######################################################################################################################
	if cakelengthreviser ==0:
		cakehcr = np.nan
	elif Pressuredropcaker < 0:
		cakehcr = np.nan
	else:
		cakehcr = Discharge2nr*cakelengthreviser/(Arear*Pressuredropcaker/10000)              #the Darcy law; the Pressure dropr(pa) was converted to the Unit(m),divived by 10000
...
##########caculate the hydraulic conductivity of filter(m/s), the permittivity(s-1) and output to the excel built before############################################################################################################### 	
	...
	Densityfilterbottom = Density[41-1,:]                                                     #the density filter at bottom value  [list]
...
############caculate the hydraulic conductivity of the system(m/s) and output to the excel built before, the system contains the cake and filter##########################################################################################
	if cakelengthreviser == 0:
		...
		systemlengthr = filterlengthr + cakelengthreviser                                           #the reality system length(m)
...
############caculate the DE##########################################################################################################################################################################################################
	...
	for i in range(MMMM-1, Np):
		if posy[i] >= 41 and posy[i] <= cakebottom + cakelengthrevise:
			M+=1
...
book.save('testfiltration.xls')                                                               #save the data in the excel
######### read the excel data that output last step and plot the figures#################################################################################################################################
wb = xlrd.open_workbook('testfiltration.xls')                                                 #open the excel
...
cols13 = table.col_values(12)
cols14 = table.col_values(13)
...
plt.xlabel("reality time t(s)")
plt.ylabel("DE")
plt.plot(cols1,cols14,alpha = 0.6)
plt.show()
