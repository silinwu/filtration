...
void Setup(LBM::Domain &dom, void *UD)
{
    ...
    for(size_t ip=0; ip<dom.Particles.size(); ++ip)
    {
       ...
       //if(dom.Particles[ip].X(1)>dat.ny-3*dat.R-1.5*dat.lll)
       //{
       //    #pragma omp atomic
       //    ipp++;
		//}
	...
	//if(ipp<(((3*dat.nfloc+2)*(dat.nfloc+1)/2)-1))

  //if(ipp<dat.fnx*dat.nfloc*(1+2*dat.nfloc))
  //{
  //     Vec3_t pos(0,0,0);
  //     Vec3_t v(0,0,0);
  //     Vec3_t w(0,0,0);
  ...
  //     {
  //         //Vec3_t dxr(random(-0.5*(0.5*lll-nfloc*2*R),0.5*(0.5*lll-nfloc*2*R)),random(-0.5*(0.5*lll-nfloc*2*R),0.5*(0.5*lll-nfloc*2*R)),0.0);
  //         for(int k=-dat.nfloc; k<(dat.nfloc+1); ++k)
  //         {
//			double px = 0.5*dat.lll+dat.R+i*(dat.lll+2*dat.R) + k*2*dat.R;
  //             for(int z=-dat.nfloc; z<(dat.nfloc+1); ++z)
  //             {    
//				double py = dat.ny - dat.lll - dat.R + z*2*dat.R;
//				pos = px, py , 0;
...
int main (int argc, char **argv) try
{
 ...
 // the filter   
    ...
    int    Pnx = 8;                             //filter particle numbers in one row
    int    Pny = 3;                             //the column numbers of filter particles
    double pdx = 38.4;                            //gap between filter particle surface in x axis
    double pdy = 38.4;                           
 // the floc   
    int    fnx = 6;                             //floc number
    int    fny = 6;
    double lll = 105;                           //the gap between particle in the middle of the floc surfaces
    double nfloc = 1;                           //the number of the surface outside the particle in the middle of the floc
	...
	//floc position and draw
        ...
        if(shape == 3)
        {
            ...
            for(int j=0; j<fny; ++j)
            {
                for(int i=0; i<fnx; ++i)
                {
                  Vec3_t dxr(random(-0.3*RRR,0.3*RRR),random(-0.3*RRR,0.3*RRR),0.0);
                  int nnfloc = nfloc;
        ...
        if(shape == 4)
        {
            ...
            for(int j=0; j<fny; ++j)
            {
                for(int i=0; i<fnx; ++i)
                {
                    Vec3_t dxr(random(-0.45*RRR,0.45*RRR),random(-0.45*RRR,0.45*RRR),0.0);
                    for(int k=-nfloc; k<nfloc+2; k=k+2)
                    {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(size_t ip=0; ip<dom.Particles.size(); ++ip)
    {
		...
		dom.Particles[ip].Lc = 1.28e-6/ratiol;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // dom.modexy = 1;
    //solving
    dom.SolveIBM( Tf, dtout, "testflocfltitration_", Setup, NULL);
}MECHSYS_CATCH